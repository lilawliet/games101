...
Vector3f Scene::castRay(const Ray &ray, int depth) const
{
    ...
    Intersection intersection = Scene::intersect(ray);      // 求一条光线与场景到交点
    if (!intersection.happened) // 没交点
        return {};
    
    if (intersection.m->hasEmission())  // 交点是光源
    ...
    */
    ...
    // ----------------- 2. 交点是物体：1）向光源采样计算 direct
    Intersection lightpos;
    float lightpdf = 0.0f;
    sampleLight(lightpos, lightpdf); // 获得对光源到采样，包括光源到位置和采样到pdf（在场景到所有光源上按面积 uniform 到 sample 一个点，并计算该 sample的概率密度）
    ...
    if (light_to_anything_ray.distance - collisionlight.norm() > 0.005)  // 没有遮挡，有横条就是数字太小
    {
    ...
    // ----------------- 2. 交点是物体： 2）向其他物体采样递归计算 indirect
