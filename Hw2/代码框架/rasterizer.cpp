...
{   
    ...
    Eigen::Vector3f p1q = {x - _v[1].x(), y - _v[1].y(), 1.0f};
    Eigen::Vector3f p2q = {x - _v[2].x(), y - _v[2].y(), 1.0f};
    Eigen::Vector3f p0q = {x - _v[0].x(), y - _v[0].y(), 1.0f};

    if(
        p0p1.cross(p0q).z() > 0 && p1p2.cross(p1q).z() > 0 && p2p0.cross(p2q).z() > 0 || 
        p0p1.cross(p0q).z() < 0 && p1p2.cross(p1q).z() < 0 && p2p0.cross(p2q).z() < 0
        )
    {
        return true;
...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    x_min = (int)floor(MIN(v[0].x(), MIN(v[1].x(), v[2].x())));
    x_max = (int) ceil(MAX(v[0].x(), MAX(v[1].x(), v[2].x())));
    y_min = (int)floor(MIN(v[0].y(), MIN(v[1].y(), v[2].y())));
    y_max = (int) ceil(MAX(v[0].y(), MAX(v[1].y(), v[2].y())));
            ...
            for(int y=y_min; y <=y_max; y++){
                ...
                if (count != 0){
                    ...
                    if (depth_buf[index] > minDepth) {
                        ...
                        set_pixel(Vector3f(x, y, minDepth), t.getColor() * count / 4.0);
                    }
                }
